...
- DOORMAN_OPENROUTER_API: Указать ключ API Openrouter для проверки профилей/сообщений через LLM
...
- DOORMAN_CHANNELS_EMOJI_DISABLE: Через запятую группы в которых пропускается простая проверка TooManyEmojis
...
- DOORMAN_CAPTCHA_DISABLE: Через запятую группы в которых новым пользователям не показывается капча
- DOORMAN_ADMIN_CHAT_MAP: В кавычках "канал=чат" (можно несколько через запятую), чтобы группировать админские чаты по разным каналам
- DOORMAN_IGNORE_REPORTED_CHANNELS: Через запятую каналы о которых не будет уведомлений в админку (дружественные или собственные)
- DOORMAN_CHANNEL_MARKETOLOGY_EXCLUSION: Через запятую каналы которые часто постят рекламу? (не уверен)
